...
const TimelineIcon: React.FC<{ type: "education" | "work" }> = ({ type }) => {
  ...
  return (
    <div className="absolute left-0 top-0 flex h-8 w-8 items-center justify-center rounded-full bg-gray-800 border-2 border-gray-700 transition-all duration-300 group-hover:border-emerald-400 group-hover:bg-emerald-400/10">
      <Icon className="h-4 w-4 text-gray-400 transition-all duration-300 group-hover:text-emerald-400" />
    </div>
  );
};
...
const TimelineItemComponent: React.FC<{
  ...
  return (
    <div className="group relative pl-10 pb-8">
      {/* Timeline line */}
      {!isLast && (
        <div className="absolute left-4 top-8 h-full w-px bg-gray-700 transition-all duration-300 group-hover:bg-emerald-400/30" />
      )}
